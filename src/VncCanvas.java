        ...
        synchronized (memImage) {
            if (rfb.framebufferWidth == scaledWidth) {
                ...
                try {
                    ...
                    ImageIO.write(image, "jpg", new File(viewer.compname + "-" + Calendar.getInstance().getTimeInMillis() + ".jpg"));
                } catch (IOException ex) {
                }*/
                paintScaledFrameBuffer(g);

                /*if (viewer.timeNext - viewer.timePresent <= 0) {
                BufferedImage image = new BufferedImage(scaledWidth, scaledHeight, BufferedImage.TYPE_INT_RGB);
                Graphics2D g2 = image.createGraphics();
                g2.drawImage(memImage, 0, 0, scaledWidth, scaledHeight, null);
                g2.dispose();
                ...
                viewer.timeNext = Calendar.getInstance().getTimeInMillis() + 1000;
                }*/
                //capture();
...
//                cursorX - hotX, cursorY - hotY, cursorWidth, cursorHeight);
    ...
    //
    synchronized MemoryImageSource decodeCursorShape(int encodingType, int width, int height)
            throws IOException {
...
//                    cursorX - hotX, cursorY - hotY, cursorWidth, cursorHeight);
    ...
    /* *
     * Added on evnctv 1.003
     *  - To add 0 in front of 0-9 on date & time
     */
    ...
    /* *
     * Added on evnctv 1.003
     *  - To replace special charactors in file name before it is saved
     */
    private String manageScreenName(String name) {
        name = name.replace('\\', '-');
        name = name.replace('/', '-');
        name = name.replace(':', '-');
        name = name.replace('*', '-');
    ...
    /* *
     * Added on evnctv 1.003
     *  - To replace \ to / in path name, Specially on Windows OS
     */
    private String manageScreenPath(String path) {
        if (!path.equals("")) {
            path += "\\";
            path = path.replace('\\', '/');
        }
